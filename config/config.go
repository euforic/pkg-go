...
func (c *Config) Load(cfg interface{}, files ...string) (err error) {
	...
	err, _ = c.load(cfg, false, files...)

	if c.Settings.AutoReload {
		go func() {
			timer := time.NewTimer(c.Settings.AutoReloadInterval)
			for range timer.C {
				...
				if err, changed = c.load(reflectPtr.Interface(), true, files...); err == nil && changed {
					...
					if c.Settings.AutoReloadCallback != nil {
						c.Settings.AutoReloadCallback(cfg)
					}
				} else if err != nil {
					fmt.Printf("Failed to reload configuration from %v, got error %v\n", files, err)
				}
				timer.Reset(c.Settings.AutoReloadInterval)
			}
		}()
	}
	return
